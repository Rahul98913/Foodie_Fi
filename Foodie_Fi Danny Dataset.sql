...
VALUES
 ...
 select monthname(start_date) as month ,count(*)  as trail_plan_customers 
 from subscriptions 
 where plan_id = 0
 group by month 
 order by trail_plan_customers desc ;
...
select plan_name , COUNT( distinct customer_id) as plan_count_after_trail ,
 ROUND(
        (COUNT(DISTINCT customer_id) / (SELECT COUNT(DISTINCT customer_id) FROM subscriptions)) * 100, 
        0
    ) AS percent_churn_after_trial
...
where rn = 2 
group by plan_name 
order  by plan_count_after_trail DESC ;
...
ROUND(
        (COUNT(DISTINCT customer_id) / (SELECT COUNT(DISTINCT customer_id) FROM subscriptions)) * 100, 
        0
    ) AS percent_plan_breakdown
FROM CTE
where rn = 2 AND start_date <= "2020-12-31"
GROUP BY plan_name
...
FROM CTE
where rn = 2 AND YEAR(start_date) = 2020 AND plan_name = 'pro annual'
GROUP BY plan_name
ORDER BY customers_upgraded_to_annual DESC ;
